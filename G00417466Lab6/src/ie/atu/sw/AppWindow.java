...
//This class handles setting up the windows 
//Extentds application to start with
//Since we also have the list of observers from the model we could say that this app is a subject and apply all rules to it
public class AppWindow extends Application {
	...
	//This is a template method and is an example of the hollywood principle -> "Don't call us, we'll call you"
	//Like a life cycle method like start and stop
	@Override
	public void start(Stage stage) throws Exception { //This is a ***Template Method***
		...
		//Things like "Button" are leaf nodes and as a result you cant add anything to them, they just exist
		...
		btnAdd.setOnAction(e -> { //Plant an observer on the button
			 ...
			 * 
			 */
			//The lambda expression takes in a user gesture, and acts as a controller access to the model
			//We then feed in the add functionality to the model - update the model
			customers.add(
		...
		//Then we just add this button to the toolbar
		...
		btnDelete.setOnAction(e -> { //Plant an observer on the button
			...
			//This is very similar to the one above, using lambda to update the view and tell it to reset itself
			Customer c = tv.getSelectionModel().getSelectedItem();
			customers.remove(c);
		});
		//We then attach the leaf node to the parent
		...
		//Adding our new created polygon drawing function to the box
	...
	private TitledPane getFileChooserPane(Stage stage) {
		...
		//This helps us filter so that we're only looking for .jar files
		FileChooser fc = new FileChooser(); //A leaf node
		fc.getExtensionFilters().addAll(new FileChooser.ExtensionFilter("JAR Files", "*.jar"));

		//We then add a new button that will fire up the ability/search functionality for files
	...
	//This helps us set up the actual model itself - we pass in the model we're using
	...
	private TableView<Customer> getTableView() {
		 ...
		 */
		//If we want to add/remove or do whatever to the view then this basically tells it to resize itself on an update
		...
		//We then need to bind each column in the view to a customer object properly
		//Explanation above explains it better - but effectively what we're doing is mapping object to a string & spitting it out in the view
		
		//This is a callback method - when called we call the method to get the cell data and wrap the name up as a simple string property
		//Wants to bind the name to column
		...
		//This does the same as ones above but this time its done in a different format
		...
		//Image is done slightly different with the image view being used 
